<div>
  ...
  <tr wire:key="{{$item['product_id']}}">
            ...
            <td class="py-4">{{ Number::currency($item['unit_amount'] * $item['quantity'], 'BGN') }}</td>
            ...
            <td><button wire:click="removeItem({{ $item['product_id'] }})"
              ...
              <span wire:loading.remove wire:target="removeItem({{ $item['product_id'] }})">Remove</span><span wire:loading wire:target="removeItem({{ $item['product_id'] }})">Removing...</span></button>
            </td>
          </tr>
  @endforeach
@elseif(is_array($cart_items) && count($cart_items) === 0)
  <tr>
    <td colspan="5" class="text-center py-4 text-slate-500 font-semibold">
        ...
        <div class="md:w-1/4">
          <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="flex justify-between mb-2">
              ...
              <span class="font-semibold">{{Number::currency($grand_total, 'BGN')}}</span>
            </div>
            @if($cart_items)
